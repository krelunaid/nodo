...
import { canOpenStyle } from "@/lib/trading/allocation";
...
export function Ticket({
  ...
  const extra = [...gate.reasons, ...halt.reasons].filter((r) => !check.reasons.includes(r));
  const blockedReasons = [...check.reasons, ...extra];
  const canSubmit = check.ok && gate.ok && halt.ok && !liveBlocked;

  function submit() {
    if (liveBlocked) return;
    if (!canSubmit || sl == null || tp == null) return;
    openTrade({
    ...
    <section className="flex flex-col gap-4">
      <div>
        <p className="text-xs font-medium tracking-wide text-muted uppercase">
          Ticket — stop sulla candela, target sul grafico
        </p>
      ...
      <dl className="grid grid-cols-2 gap-2 rounded-md border border-border bg-bg p-3 text-xs">
        <Row label="Puntata" value={formatMoney(check.riskAmount)} />
        <Row
          label="R:R"
          value={check.rr == null ? "—" : `${check.rr.toFixed(2)}`}
        />
        <Row label="Size" value={check.size.toFixed(2)} />
        <Row label="Obiettivo" value={formatMoney(check.rewardAmount)} />
      </dl>
